...
- ### 에러 및 처리
  - **[HPM] Error occurred while trying to proxy request /api/signup from localhost:3000 to localhost:5000**
    - 이런 에러처럼 만약 나온다면 axios의 공식 홈페이지나 npm의 axios를 사용하는 법을 찾아본 뒤 똑같이 했을 경우에도 같은 에러가 나온다면 그 땐 axios 버전의 문제였다.
  - **TypeError: styled_components**WEBPACK_IMPORTED_MODULE_2**.default.Link is not a function**
